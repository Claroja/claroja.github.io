next的配置文件在:
1. `themes/next/_config.yml`(git方式安装)
2. `node_modules/hexo-theme-next/_config.yml`(npm方式安装)
3. `_config.next.yml`(将1,2两种方式文件移动到根目录, 作为自定义配置)

在hexo中开启next, 进入hexo的`_config.yml`:
```yml
theme: next
```
